    ...
    , UITextViewDelegate
{
    ...
    public func removeTag(tag: String, animated: Bool) {
        ...
        for (idx, value) in tagsMade.enumerate() {
            if tag == value {
                debugPrint("Removed", terminator: "")   // MARK: Replace the "FOUND" with "Removed" - by @cirno.mainasuk@gmail.com
                foundIndex = idx
            }
        }
    ...
    func tagButtonDidPushed(sender: AnyObject!) {
        ...
        if deleteButton.hidden == false && btn.tag == deleteButton.tag {
            ...
            if deleteButton.superview == nil {
                ...
                scrollView.bringSubviewToFront(deleteButton)
            }
    ...
    private func initControls() {
        ...
        scrollView.contentInset = UIEdgeInsetsMake(0, 8, 0, 0)
        ...
        tagInputView.textContainer.maximumNumberOfLines = 1
        tagInputView.textContainer.lineBreakMode = NSLineBreakMode.ByTruncatingHead
